...
#### One-line summary
Gets desktop users to the right download path faster and lets `sprintnex-orchestrator` recover when npm skips its platform binary.
...
- `sprintnex-orchestrator` postinstall now downloads the matching release binary when optional platform packages are missing.
