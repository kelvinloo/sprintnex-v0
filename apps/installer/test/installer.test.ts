...
describe("desktopBootstrapPath", () => {
  test("honors the explicit override", () => {
    expect(
      desktopBootstrapPath(
        { OPENWORK_DESKTOP_BOOTSTRAP_PATH: "/tmp/custom.json" },
        "darwin",
      ),
  ...
  test("prefers XDG_CONFIG_HOME on every platform", () => {
    expect(desktopBootstrapPath({ XDG_CONFIG_HOME: "/xdg" }, "linux")).toBe(
      path.join("/xdg", "openwork", "desktop-bootstrap.json"),
    );
    expect(desktopBootstrapPath({ XDG_CONFIG_HOME: "/xdg" }, "win32")).toBe(
      path.join("/xdg", "openwork", "desktop-bootstrap.json"),
    );
  });
  ...
  test("uses LOCALAPPDATA on Windows and ~/.config elsewhere", () => {
    ...
    expect(desktopBootstrapPath({}, "darwin")).toBe(
  ...
  test("resolves the legacy bootstrap path under ~/.config on every platform", () => {
    ...
    expect(
      legacyDesktopBootstrapPath({ USERPROFILE: "C:\\Users\\u" }, "win32"),
    ).toBe(
      path.join(
        "C:\\Users\\u",
        ".config",
...
describe("releaseAssetFor", () => {
  test("resolves per-platform asset names", () => {
    expect(releaseAssetFor("v0.17.7", "darwin", "arm64").fileName).toBe(
      "sprintnex-mac-arm64-0.17.7.dmg",
    );
    expect(releaseAssetFor("0.17.7", "darwin", "x64").fileName).toBe(
      "sprintnex-mac-x64-0.17.7.dmg",
    );
    expect(releaseAssetFor("0.17.7", "win32", "x64").fileName).toBe(
      "sprintnex-win-x64-0.17.7.exe",
    );
    expect(releaseAssetFor("0.17.7", "linux", "x64").fileName).toBe(
      "sprintnex-linux-x64-0.17.7.AppImage",
    );
    expect(releaseAssetFor("0.17.7", "linux", "arm64").fileName).toBe(
      "sprintnex-linux-arm64-0.17.7.AppImage",
    );
  });

  test("builds the release download URL from the version tag", () => {
    expect(releaseAssetFor("0.17.7", "darwin", "arm64").url).toBe(
      "https://github.com/k3lvinlkf/openwork/releases/download/v0.17.7/sprintnex-mac-arm64-0.17.7.dmg",
    );
  });
...
describe("resolveInstallerConfig", () => {
  test("reads env overrides and normalizes URLs", async () => {
    const { config, source } = await resolveInstallerConfig({
      env: {
  ...
  test("accepts an optional logo URL and rejects non-http logos", async () => {
    const { config } = await resolveInstallerConfig({
      env: {
  ...
  test("fails without a configured deployment", async () => {
    await expect(
      resolveInstallerConfig({
        env: {},
        execPath: path.join(os.tmpdir(), "openwork-installer"),
      }),
    ).rejects.toThrow();
  });
  ...
  test("prefers env overrides over sidecar config", async () => {
    const dir = mkdtempSync(
      path.join(os.tmpdir(), "openwork-installer-precedence-"),
    );
    try {
      const execPath = path.join(dir, "openwork-installer");
      writeFileSync(execPath, "");
      writeFileSync(
        path.join(dir, "openwork-installer.json"),
        JSON.stringify({
          clientName: "Sidecar",
  ...
  test.skipIf(process.platform === "win32")(
    "reads sidecar next to the enclosing app bundle",
    async () => {
      const dir = mkdtempSync(
        path.join(os.tmpdir(), "openwork-installer-app-sidecar-"),
      );
      try {
...
describe("macOS App Translocation helpers", () => {
  test("parses a normal mount table line", () => {
    expect(
      parseMountTableLine(
        "/private/tmp/OpenWork Installer.app on /private/var/folders/abc/T/AppTranslocation/123 (nullfs, local, read-only)",
      ),
    ).toEqual({
  ...
  test("parses paths with spaces and on in the source", () => {
    expect(
      parseMountTableLine(
        "/private/tmp/folder with spaces/source on disk/OpenWork Installer.app on /private/var/folders/abc/T/AppTranslocation/UUID With Space (nullfs, local)",
      ),
    ).toEqual({
      source:
        "/private/tmp/folder with spaces/source on disk/OpenWork Installer.app",
  ...
  test("ignores junk mount table lines", () => {
    ...
    expect(
      parseMountTableLine(
        "/private/tmp/OpenWork Installer.app on /private/var/folders/abc/T/AppTranslocation/123",
      ),
    ).toBeNull();
  });
  ...
  test("resolves the original app through the translocated /d path", () => {
    ...
    expect(
      resolveTranslocatedOriginalPath(
        execPath,
        `${source} on ${mountPoint} (nullfs, local, nodev)\n`,
      ),
    ).toBe(source);
  });
  ...
  test("skips non-nullfs mounts", () => {
    ...
    expect(
      resolveTranslocatedOriginalPath(
        execPath,
        `${source} on ${mountPoint} (apfs, local)\n`,
      ),
    ).toBeNull();
  });
  ...
  test("requires a mountpoint path-prefix boundary", () => {
    ...
    expect(
      resolveTranslocatedOriginalPath(
        execPath,
        `${source} on ${mountPoint} (nullfs, local)\n`,
      ),
    ).toBeNull();
  });
  ...
  test("returns null when no translocation mount matches", () => {
    const execPath =
      "/private/var/folders/abc/T/AppTranslocation/123/d/OpenWork Installer.app/Contents/MacOS/openwork-installer";
    const mountTable =
      "/private/tmp/OpenWork Installer.app on /private/var/folders/abc/T/AppTranslocation/other (nullfs, local)\n";
  ...
  test("detects App Translocation paths", () => {
    ...
    expect(
      isTranslocatedPath(
        "/Applications/OpenWork Installer.app/Contents/MacOS/openwork-installer",
      ),
    ).toBe(false);
  });
  ...
  test("reads the sidecar next to the original translocated app", () => {
    const dir = mkdtempSync(
      path.join(os.tmpdir(), "openwork-installer-translocated-"),
    );
    try {
      ...
      expect(
        readSidecarConfig({
          execPath,
          readMountTable: () =>
            `${originalAppPath} on ${mountPoint} (nullfs, local, read-only)\n`,
          warn: () => undefined,
        }),
      ).toEqual({
        appName: "OpenWork",
  ...
  test("falls through when the translocation mount is missing", () => {
    const dir = mkdtempSync(
      path.join(os.tmpdir(), "openwork-installer-translocated-missing-"),
    );
    try {
      const originalAppPath = path.join(dir, "OpenWork Installer.app");
      const execPath =
      ...
      expect(
        readSidecarConfig({
          execPath,
          readMountTable: () =>
            `${originalAppPath} on /private/var/folders/abc/T/AppTranslocation/other (nullfs, local)\n`,
          warn: () => undefined,
        }),
      ).toBeNull();
    } finally {
...
describe("install link helpers", () => {
  test("parses filename stamps and install config URLs", () => {
    expect(
      parseInstallerFilenameTag(
        "OpenWork-Installer--127.0.0.1_8790--abcDEF12.exe",
      ),
    ).toEqual({
      host: "127.0.0.1:8790",
      token: "abcDEF12",
    ...
    expect(
      parseInstallerFilenameTag(
        "OpenWork-Installer--api.example.com--abcDEF12",
      ),
    ).toEqual({
      host: "api.example.com",
      token: "abcDEF12",
    });
    expect(installConfigUrlFor("127.0.0.1:8790", "abcDEF12")).toBe(
      "http://127.0.0.1:8790/v1/install-config?token=abcDEF12",
    );
    expect(installConfigUrlFor("api.example.com", "abcDEF12")).toBe(
  ...
  test("parses pasted install-link inputs", () => {
    expect(
      parseInstallLinkInput("https://app.example.com/install?token=abcDEF12")
        ?.url,
    ).toBe("https://app.example.com/api/den/v1/install-config?token=abcDEF12");
    expect(
      parseInstallLinkInput(
        "https://api.example.com/v1/install-config?token=abcDEF12",
      )?.url,
    ).toBe("https://api.example.com/v1/install-config?token=abcDEF12");
    expect(parseInstallLinkInput("api.example.com abcDEF12")?.url).toBe(
      "https://api.example.com/v1/install-config?token=abcDEF12",
    );
    expect(
      parseInstallLinkInput("http://api.example.com/install?token=abcDEF12"),
    ).toBeNull();
  });
});
...
describe("writeBootstrapConfig", () => {
  test("migrates a legacy organization config instead of replacing it with hosted defaults", () => {
    ...
    try {
      ...
      expect(parsed.baseUrl).toBe(
        "https://openwork.organization.internal.example",
      );
      expect(parsed.apiBaseUrl).toBe(
        "https://api.organization.internal.example",
      );
  ...
  test("replaces an installed hosted default with a custom organization config", () => {
    ...
    try {
      ...
      writeBootstrapConfig(
        {
          ...
          requireSignin: true,
          logoUrl:
            "https://openwork.custom.internal.example/assets/wordmark.svg",
        },
        env,
        "win32",
      );
      ...
      expect(parsed.brandLogoUrl).toBe(
        "https://openwork.custom.internal.example/assets/wordmark.svg",
      );
      expect(parsed.prepared).toEqual({ orgId: "org_example" });
      expect(parsed.claimLinks).toEqual([{ id: "claim_example" }]);
