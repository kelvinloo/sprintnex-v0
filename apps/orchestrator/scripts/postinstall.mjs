...
function packageName() {
  ...
  return `sprintnex-orchestrator-${platform}-${arch}`
}
...
function readOwnVersion() {
  ...
  if (!version) {
    throw new Error("sprintnex-orchestrator: package version is missing")
  }
  return version
}

function resolveFallbackBaseUrl(version) {
  const override = String(process.env.SPRINTNEX_ORCHESTRATOR_DOWNLOAD_BASE_URL || "").trim()
  if (override) {
    return override.replace(/\/$/, "")
  }
  return `https://github.com/different-ai/openwork/releases/download/sprintnex-orchestrator-v${version}`
}
...
async function downloadFallbackBinary() {
  ...
  console.log(`sprintnex-orchestrator: downloading fallback binary ${asset}`)
  ...
  console.log(`sprintnex-orchestrator: installed fallback binary at ${destination}`)
}
...
async function main() {
  try {
    ...
    console.log(`sprintnex-orchestrator: verified platform package: ${pkg}`)
    return
  } catch {
    if (existsSync(fallbackBinaryPath())) {
      console.log(`sprintnex-orchestrator: using existing fallback binary at ${fallbackBinaryPath()}`)
      return
    }
  }
  ...
  try {
    ...
    console.error(
      `sprintnex-orchestrator: failed to locate platform binary package (${pkg}).\n` +
