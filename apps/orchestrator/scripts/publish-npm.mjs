...
for (const target of targets) {
  ...
  const src = join(root, "dist", "bin", `sprintnex-${target.bun}${ext}`);
  if (!existsSync(src)) {
    throw new Error(
      `Missing sprintnex binary at ${src}. Run: pnpm --filter sprintnex-orchestrator build:bin:all`,
    );
  }
  ...
  writeJson(join(dir, "package.json"), {
    name,
    version,
    description: "Platform binary for sprintnex-orchestrator",
    license: "MIT",
    os: [target.os],
    cpu: [target.cpu],
    bin: {
      openwork: `./bin/openwork${ext}`,
      "sprintnex-orchestrator": `./bin/openwork${ext}`,
    },
    files: ["bin"],
  });
...
const meta = join(outroot, "sprintnex-orchestrator");
...
published.push({ name: "sprintnex-orchestrator", dir: meta });
