...
export async function resolveServerConfig(cli: CliArgs): Promise<ServerConfig> {
  const envConfigPath = process.env.SPRINTNEX_SERVER_CONFIG;
  ...
  const envWorkspaces = parseList(process.env.SPRINTNEX_WORKSPACES);
  ...
  const envOpencodeBaseUrl = process.env.SPRINTNEX_OPENCODE_BASE_URL;
  const envOpencodeDirectory = process.env.SPRINTNEX_OPENCODE_DIRECTORY;
  const envOpencodeUsername = process.env.SPRINTNEX_OPENCODE_USERNAME;
  const envOpencodePassword = process.env.SPRINTNEX_OPENCODE_PASSWORD;
  ...
  const tokenFromEnv = process.env.SPRINTNEX_TOKEN;
  const hostTokenFromEnv = process.env.SPRINTNEX_HOST_TOKEN;
  ...
  const approvalTimeoutMs =
    ...
    (process.env.SPRINTNEX_APPROVAL_TIMEOUT_MS ? Number(process.env.SPRINTNEX_APPROVAL_TIMEOUT_MS) : undefined) ??
  ...
  const envCorsOrigins = process.env.SPRINTNEX_CORS_ORIGINS;
  ...
  const envReadOnly = process.env.SPRINTNEX_READONLY;
  ...
  const envLogFormat = process.env.SPRINTNEX_LOG_FORMAT;
  const logFormat =
    cli.logFormat ??
  ...
  const envLogRequests = parseBoolean(process.env.SPRINTNEX_LOG_REQUESTS);
  ...
  const host = cli.host ?? process.env.SPRINTNEX_HOST ?? fileConfig.host ?? DEFAULT_HOST;
  const port = cli.port ?? (process.env.SPRINTNEX_PORT ? Number(process.env.SPRINTNEX_PORT) : undefined) ?? fileConfig.port ?? DEFAULT_PORT;
