...
const DEFAULT_TEXT_COMMAND =
  "Summarize the current Sprintnex session and put the next step in the composer.";
...
const TOOL_LABELS: Record<string, string> = {
  openwork_snapshot: "Checking Sprintnex",
...
function safeJson(value: unknown) {
  try {
    ...
    return JSON.stringify({
      ok: false,
      error: "Could not serialize tool result",
    });
  }
}

function humanToolLabel(toolName?: string) {
  if (!toolName) return "Sprintnex action";
...
async function loadVoiceSessionContext(
...
function setMicDiagnostics(stream: MediaStream | null) {
  ...
  setVoiceRuntimeSnapshot((current) => ({
    ...current,
    micDiagnostics: describeAudioTrack(track),
  }));
}
...
async function requestMacMicrophoneAccess() {
  ...
  setVoiceRuntimeSnapshot((current) => ({
    ...current,
    micDiagnostics: `macOS microphone permission is ${status}.`,
  }));
  return result.granted;
}

async function executeOpenWorkTool(
  name: string,
  args: Record<string, unknown>,
) {
  const control = window.__openworkControl;
  if (!control)
    return { ok: false, error: "Sprintnex control surface is not available." };

  if (name === "openwork_snapshot")
    return { ok: true, snapshot: control.snapshot() };
  if (name === "openwork_list_actions")
  ...
  if (entry.role === "assistant") {
    return (
      <article className="mr-8 rounded-2xl border border-border bg-card px-3 py-2 text-sm leading-relaxed text-card-foreground shadow-sm">
        {entry.error ? (
          <div className="mb-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-destructive">
            Error
          </div>
        ) : null}
...
export function VoicePanel(props: VoicePanelProps) {
        ...
        statusText:
          ...
          (nextStatus === "connecting"
            ...
            : nextStatus === "listening"
              ? "Listening. Ask OpenWork to act."
              : nextStatus === "speaking"
                ? "OpenWork is speaking..."
                : nextStatus === "muted"
                  ? "Connected, microphone muted."
                  : nextStatus === "error"
                    ? "Voice Mode needs attention."
  ...
  const connectRealtime = useCallback(
    async (audioInput = true) => {
      ...
      const sessionContext = await loadVoiceSessionContext(
        client,
        props.workspaceId,
        props.sessionId,
      );
      ...
      if (audioInput) {
        ...
        setVoiceRuntimeSnapshot((current) => ({
          ...current,
          micDiagnostics:
            "Voice command is using typed or injected audio, not the microphone.",
        }));
      ...
      addEntry(
        "system",
        `Realtime connected with ${realtimeSession.model} and ${realtimeSession.tools.length} OpenWork tools.`,
      );
  ...
  const sendTextCommand = useCallback(
    async (text: string) => {
      ...
      if (!channel || channel.readyState !== "open")
        return { ok: false, error: "Realtime channel is not open." };
      addEntry("user", value);
      channel.send(
        JSON.stringify({
          type: "conversation.item.create",
  ...
  const injectTranscript = useCallback(
    async (args: unknown) => {
      ...
      window.dispatchEvent(
        new CustomEvent("openwork:voice-transcript", { detail: { text } }),
      );
      ...
      args: [
        {
          ...
          type: "string",
          required: true,
          description: "Transcript text to inject.",
        },
      ],
      ...
      args: [
        {
          ...
          type: "string",
          required: true,
          description: "Text command to send through the Realtime model.",
        },
      ],
      previewArgs: { text: DEFAULT_TEXT_COMMAND },
      targetRef: panelRef,
      ...
      args: [
        {
          ...
          type: "string",
          required: true,
          description: "Base64 encoded PCM16 mono audio.",
        },
      ],
      targetRef: panelRef,
      execute: injectAudio,
          ...
          <div className="flex flex-col gap-5 px-4 py-5">
            ...
            <div className="text-center">
              ...
              <div className="mt-1 text-xs text-muted-foreground">
                Say things like "type a follow-up", "send it", or "read the
            ...
            {!props.client ? (
              <Card variant="outline" size="sm">
                ...
                <CardContent className="text-sm text-muted-foreground">
                  Voice Mode needs the local OpenWork server so it can mint
                  short-lived Realtime client secrets without exposing your API
                  key to the renderer.
                </CardContent>
              </Card>
            ) : null}
            ...
            <div className="flex flex-col gap-2">
              ...
              {entries.length ? (
                ...
                <div className="rounded-2xl border border-dashed border-border px-4 py-5 text-center text-sm text-muted-foreground">
                  Start voice or inject a transcript from UI MCP to see the
                  voice timeline.
                </div>
              )}
