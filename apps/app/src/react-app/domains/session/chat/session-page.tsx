...
export type SessionPageProps = {
  ...
  sprintnexTaskExecuting?: boolean;
...
function sessionExistsInWorkspace(
  groups: WorkspaceSessionGroup[],
  workspaceId: string,
...
function hiddenAccessibleTargetsStorageKey(
...
function readHiddenAccessibleTargetIds(
  workspaceId: string | null | undefined,
  sessionId: string | null | undefined,
): Set<string> {
...
function writeHiddenAccessibleTargetIds(
  workspaceId: string | null | undefined,
  sessionId: string | null | undefined,
  ids: Set<string>,
) {
...
export function SessionPage(props: SessionPageProps) {
  ...
  const sessionSidePanel = useUiStateStore((state) =>
    props.selectedSessionId
      ? (state.sidePanelState[props.selectedSessionId] ?? null)
      : null,
  );
  ...
  const transcriptTargets = usePanelTabStore((state) =>
    props.selectedSessionId
      ? (state.transcriptArtifactTargets[props.selectedSessionId] ??
        EMPTY_TRANSCRIPT_TARGETS)
      : EMPTY_TRANSCRIPT_TARGETS,
  );
  ...
  ] = useState<PendingConversationHistoryNavigation | null>(null);
  const [splitSessionId, setSplitSessionId] = useState<string | null>(null);
  const [createGroupOpen, setCreateGroupOpen] = useState(false);
  const [createGroupLabel, setCreateGroupLabel] = useState("");
  const [createGroupWorkspaceId, setCreateGroupWorkspaceId] = useState<
    string | null
  >(null);
  ...
  const downloadOpenTarget = useCallback(
    async (target: OpenTarget) => {
      ...
      const result = await props.openworkServerClient.downloadWorkspaceFile(
        props.runtimeWorkspaceId,
        target.value,
      );
  ...
  const openTarget = useCallback(
    (
      ...
      if (
        ...
        const path = absoluteWorkspacePath(
        ...
        if (isOpenableFileTarget(target)) {
          if (props.selectedWorkspaceDisplay.workspaceType === "remote") {
            ...
            void openDesktopPath(
              absoluteWorkspacePath(props.selectedWorkspaceRoot, target.value),
            ).catch(() => undefined);
          }
        }
        return;
  ...
  const openBrowserRailPane = useCallback(() => {
    ...
    if (opening && isElectronRuntime()) {
      const hasBrowserTab = sessionPanelState.tabs.some(
  ...
  const openBrowserUrlControlAction = useMemo<OpenworkControlAction>(
    () => ({
      ...
      description:
        "Create or select an OpenWork built-in browser tab, navigate it to a URL, and return the CDP handle for browser automation.",
      ...
      args: [
        ...
        },
        {
          ...
          type: "string",
          description:
            "Browser provider. Use builtin or auto. External is reserved for future support.",
        },
      ],
      ...
      execute: async (args) => {
        ...
        if (provider !== "auto" && provider !== "builtin") {
          return {
            ok: false,
            error: `Browser provider is not available yet: ${provider}`,
          };
        }
        setCurrentSidePanel("panel");
  ...
  const setBrowserProxyControlAction = useMemo<OpenworkControlAction>(
    () => ({
      ...
      description:
        "Route all built-in browser traffic through an HTTP/SOCKS proxy (e.g. to browse from another location). Applies to every built-in browser tab until cleared. Pass an empty proxy to restore system network settings.",
      sideEffect: "mutation",
      args: [
        {
          name: "proxy",
  ...
  const openArtifactRailPane = useCallback(() => {
    ...
    const activeTab = sessionPanelState.tabs.find(
      (tab) => tab.id === sessionPanelState.activeTabId,
    );
    const artifactTargetIds = new Set(
      artifactFileTargets.map((target) => target.id),
    );
    const artifactTab = sessionPanelState.tabs.find(
      (tab) => tab.type === "artifact" && artifactTargetIds.has(tab.id),
    );
  ...
  const removeAccessibleTarget = useCallback(
    (target: OpenTarget) => {
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("openwork-close-right-pane", handler);
  }, [setCurrentSidePanel]);
  useEffect(() => {
  ...
  useEffect(() => {
    if (pendingConversationHistoryNavigation) {
      if (
        pendingConversationHistoryNavigation.history.workspaceId ===
          props.selectedWorkspaceId &&
        pendingConversationHistoryNavigation.targetSessionId ===
          props.selectedSessionId
      ) {
      ...
      if (
        pendingConversationHistoryNavigation.fromWorkspaceId ===
          props.selectedWorkspaceId &&
        pendingConversationHistoryNavigation.fromSessionId ===
          props.selectedSessionId
      ) {
        return;
      }
  ...
  useEffect(() => {
    ...
    const id = window.setTimeout(() => {
      setPendingConversationHistoryNavigation((current) =>
        current === pendingConversationHistoryNavigation ? null : current,
      );
    }, 5000);
  ...
  useEffect(() => {
    setSessionTabs((current) => {
      const currentWorkspaceTabs = current.filter(
        (tab) => tab.workspaceId === props.selectedWorkspaceId,
      );
      const next =
        props.selectedSessionId &&
        !currentWorkspaceTabs.some(
          (tab) => tab.sessionId === props.selectedSessionId,
        )
          ? [
  ...
  const providerCount = props.hasUsableModel
    ? 1
  ...
  const canRenderSplitSurface = Boolean(
    canRenderReactSurface &&
    splitSessionId &&
    splitSessionId !== props.selectedSessionId,
  );
  const findButtonSessionId = props.selectedSessionId;
  const canGoBackInConversationHistory =
    !pendingConversationHistoryNavigation &&
    canNavigateSelectedConversationHistory(
      conversationHistory,
      props.selectedWorkspaceId,
  ...
  const canGoForwardInConversationHistory =
    !pendingConversationHistoryNavigation &&
    canNavigateSelectedConversationHistory(
      conversationHistory,
      props.selectedWorkspaceId,
  ...
  const closeSessionTab = useCallback(
    (sessionId: string) => {
      const nextTab = sessionTabs.find(
        (tab) =>
          tab.sessionId !== sessionId &&
          tab.workspaceId === props.selectedWorkspaceId,
      );
      setSessionTabs((current) =>
  ...
  const navigateConversationHistory = useCallback(
    (direction: ConversationHistoryDirection) => {
      if (
        ...
        return;
      const next = navigateConversationTabHistory(
        conversationHistory,
        direction,
      );
  ...
  const openRenameModal = (sessionId: string) => {
    ...
    setRenameTitle(
      sessionTitleForId(props.sidebar.workspaceSessionGroups, sessionId),
    );
    setRenameOpen(true);
  };
    ...
    <div className="flex h-full min-h-0 flex-col bg-[radial-gradient(circle_at_top,rgba(74,111,255,0.12),transparent_42%),var(--app-bg,#0b1020)] text-dls-text mac:bg-transparent">
      <SidebarProvider
        ...
        className={cn(
          ...
          !shellConfig.sidebar &&
            "**:data-[slot=sidebar-container]:hidden **:data-[slot=sidebar-gap]:hidden",
        )}
      ...
      >
        <AppSidebar
          ...
          onOpenProjectTasks={props.sidebar.onOpenProjectTasks}
          onOpenRenameSession={
          ...
          <div className="flex min-h-0 flex-1">
                              ...
                              <div className="flex h-10 shrink-0 items-center gap-1 border-b border-border bg-background/80 px-2 mac:backdrop-blur-xl">
                                ...
                                >
                                    ...
                                    <TooltipTrigger
                                      render={
                                        <Button
                                          ...
                                          disabled={
                                            !canGoBackInConversationHistory
                                          }
                                          onClick={() =>
                                            navigateConversationHistory("back")
                                          }
                                        >
                                          <ArrowLeft size={14} />
                                        </Button>
                                  ...
                                  <Tooltip>
                                    <TooltipTrigger
                                      render={
                                        <Button
                                          ...
                                          disabled={
                                            !canGoForwardInConversationHistory
                                          }
                                          onClick={() =>
                                    ...
                                    />
                                    <TooltipContent>
                                      Forward in conversation history
                                    </TooltipContent>
                                  </Tooltip>
                                </div>
                                <div className="flex min-w-0 flex-1 items-center gap-1 overflow-x-auto">
                                  {sessionTabs.map((tab) => {
                                    ...
                                    const active =
                                      tab.sessionId === props.selectedSessionId;
                                    const split =
                                      tab.sessionId === splitSessionId;
                                    return (
                                      <div
                                        key={tab.sessionId}
                                        data-session-tab-id={tab.sessionId}
                                        data-session-tab-active={
                            ...
                            {props.notFoundMessage ? (
                              ...
                              <div className="px-6 py-16">
                                <div className="mx-auto max-w-lg rounded-2xl border border-red-7/35 bg-red-1/40 p-5 text-left shadow-[var(--dls-card-shadow)]">
                                  <div className="text-sm font-medium text-red-11">
                                  ...
                                  <div className="space-y-2">
                                      ...
                                      >
                                        ...
                                        <div>
                                          ...
                                          <div className="mt-0.5 text-[11px] text-dls-secondary">
                                            ...
                                            OpenAI, Google, or other providers
                                          </div>
                                        </div>
                                      </button>
                                    ) : null}
                                    ...
                                    >
                                      ...
                                      <div>
                                        ...
                                        <div className="mt-0.5 text-[11px] text-dls-secondary">
                                          Create a sample spreadsheet with
                                          customer data
                                        </div>
                                      </div>
                                    </button>
                                    ...
                                    >
                                      ...
                                      <div>
                                        ...
                                        <div className="mt-0.5 text-[11px] text-dls-secondary">
                                          Search Craigslist for couches and list
                                          the results
                                        </div>
                                      </div>
                                    </button>
                                    ...
                                    >
                                      ...
                                      <div>
                                        ...
                                        <div className="mt-0.5 text-[11px] text-dls-secondary">
                                          Add MCP servers, plugins, and
                                          integrations
                                        </div>
                                      </div>
                                    </button>
                                  </div>
                                  {props.sprintnexTaskExecuting ? (
                                    <div className="rounded-xl border border-amber-7/30 bg-amber-2/40 px-4 py-3 text-sm text-amber-11">
                                      <div className="flex items-center gap-2">
                                        <span className="size-2 rounded-full bg-amber-9 animate-pulse" />
                                        Sprintnex task is executing — chat is
                                    ...
                                    <div className="pt-4">
                                      <button
                                        className="flex h-10 w-full items-center justify-center gap-2 rounded-xl border border-dashed border-dls-border bg-dls-surface text-sm font-medium text-dls-secondary transition-colors hover:border-dls-text hover:text-dls-text"
                                        onClick={() =>
                                          props.sidebar.onCreateTaskWithPrompt?.(
                                            props.selectedWorkspaceId,
                                            "",
            ...
            <aside className="flex w-11 shrink-0 flex-col items-center gap-1 border-l border-border bg-background/95 px-1 py-2 text-muted-foreground mac:titlebar-no-drag">
              ...
              <Button
                ...
                className={cn(
                  "rounded-xl transition-colors hover:bg-muted hover:text-foreground",
                  extensionsRailActive &&
                    "bg-primary/10 text-primary hover:bg-primary/15 hover:text-primary",
                )}
                onClick={
                  props.settingsSlot
                    ? openExtensionsRailPane
                    : props.onOpenSettings
                }
                title="Extensions"
      ...
      <Dialog
        ...
        onOpenChange={(open) => {
          if (!open) setCreateGroupOpen(false);
        }}
      >
              ...
              if (e.key === "Enter" && createGroupLabel.trim()) {
                if (createGroupWorkspaceId)
                  useSessionManagementStore
                    .getState()
                    .createGroup(
                      createGroupWorkspaceId,
                      createGroupLabel.trim(),
                    );
