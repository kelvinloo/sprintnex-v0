...
type SessionActivityStore = {
  ...
  markAssistantOutput: (
    ...
    sessionId: string,
    messageId?: string,
    options?: { allowUnknownMessageRole?: boolean },
  ) => void;
  setWaitingRequest: (
    workspaceId: string,
    sessionId: string,
...
function statusForRecord(record: SessionActivityRecord): SessionActivityStatus {
  ...
  if (
    ...
    return "waiting";
  if (record.polling) return "polling";
...
function updateRecord(
  ...
  return {
    ...
    statusesByWorkspaceId: updateWorkspaceStatus(
...
export const useSessionActivityStore = create<SessionActivityStore>(
  (set, get) => ({
        ...
        for (const session of sessions) {
          ...
          nextState = {
            ...
            ...updateRecord(nextState, id, sessionId, (record) => {
              ...
              const runActive =
                normalized === "running" || normalized === "retry";
              ...
              return {
                ...
                waitingPermissionIds: runActive
    ...
    seedSessionRun: (workspaceId, sessionId, status, assistantOutput) => {
      ...
      set((state) =>
        updateRecord(state, workspace, session, (record) => {
          const normalized = normalizeRunStatus(status);
      ...
      set((state) =>
        updateRecord(state, workspace, session, (record) => {
          ...
          return {
            ...
            runActive,
            assistantOutput:
              runActive && record.runActive ? record.assistantOutput : false,
            errorActive: runActive ? false : record.errorActive,
            errorMessage: runActive ? null : record.errorMessage,
            compacting: runActive ? record.compacting : false,
            waitingPermissionIds: runActive ? record.waitingPermissionIds : [],
    ...
    markAssistantOutput: (workspaceId, sessionId, messageId, options) => {
      ...
      set((state) =>
        updateRecord(state, workspace, session, (record) => {
          ...
          if (
            message &&
            record.messageRoles[message] &&
            record.messageRoles[message] !== "assistant"
          )
            return record;
          if (
            message &&
            !record.messageRoles[message] &&
            options?.allowUnknownMessageRole !== true
          )
            return record;
      ...
      set((state) =>
        updateRecord(state, workspace, session, (record) => {
          ...
          return {
            ...record,
            [key]: waiting
              ? addValue(record[key], request)
              : removeValue(record[key], request),
          };
        }),
      );
    },
    ...
    setPolling: (workspaceId, sessionId, polling) => {
      const workspace = workspaceId.trim();
      const session = sessionId.trim();
      if (!workspace || !session) return;
      set((state) =>
    ...
    removeSession: (workspaceId, sessionId) => {
      ...
      set((state) => {
        ...
        if (!workspaceRecords?.[session] && !workspaceStatuses?.[session])
          return state;
...
export function getSessionActivityStatusLabel(status: SessionActivityStatus) {
  ...
  if (status === "polling") return t("session.listening_to_updates");
