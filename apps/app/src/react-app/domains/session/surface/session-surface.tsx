...
import {
  isAttachmentFileReadable,
  resolveAttachmentFileMetadata,
} from "@/react-app/domains/session/sync/attachment-file-part";
...
export type SessionSurfaceProps = {
  ...
  onSaveTestPlan?: (messageId: string, text: string) => Promise<void> | void;
  onOpenTarget?: (
    target: OpenTarget,
...
function isSessionSurfaceMounted(sessionId: string) {
  for (const surface of document.querySelectorAll(SESSION_SURFACE_SELECTOR)) {
    if (surface.getAttribute("data-session-surface-id") === sessionId)
      return true;
  }
  return false;
}
...
function firstMountedSessionSurfaceId() {
  return (
    document
      .querySelector(SESSION_SURFACE_SELECTOR)
      ?.getAttribute("data-session-surface-id") ?? null
  );
}
...
function resolveFindOwnerSessionId() {
  ...
  const focusedSessionId =
    focusedRoot?.getAttribute("data-session-surface-id") ?? null;
  if (focusedSessionId) return focusedSessionId;

  const lastFocusedSessionId =
    useSessionFindStore.getState().lastFocusedSessionId;
...
function statusLabel(
  ...
  if (snapshot?.status.type === "retry")
    return `Retrying: ${snapshot.status.message}`;
  return "Ready";
}
  ...
  return (
    <div className="overflow-hidden border-b border-dls-border bg-transparent">
      ...
      >
        ...
        <Minimize2
          size={12}
          className={`text-gray-8 transition-transform ${expanded ? "" : "rotate-180"}`}
        />
      </button>
      {expanded ? (
...
export function SessionSurface(props: SessionSurfaceProps) {
  ...
  const setFindLastFocused = useSessionFindStore(
    (state) => state.setLastFocused,
  );
  const findOwned = findOpen && findSessionId === props.sessionId;
  const findHighlightQuery =
    findOwned && findAppliedQuery.trim().length >= 2 ? findAppliedQuery : "";
  ...
  const inputHistory = useComposerStateStore((state) =>
    getComposerHistory(state, props.sessionId),
  );
  const appendComposerHistory = useComposerStateStore(
    (state) => state.appendHistory,
  );
  ...
  // session.
  const queuedDrafts = useComposerStateStore((state) =>
    getComposerQueuedDrafts(state, props.sessionId),
  );
  const appendQueuedDraft = useComposerStateStore(
    (state) => state.appendQueuedDraft,
  );
  const removeQueuedDraftFromStore = useComposerStateStore(
    (state) => state.removeQueuedDraft,
  );
  const clearQueuedDrafts = useComposerStateStore(
    (state) => state.clearQueuedDrafts,
  );
  const prependQueuedDrafts = useComposerStateStore(
    (state) => state.prependQueuedDrafts,
  );
  ...
  useEffect(() => {
    usePanelTabStore
      .getState()
      .syncTranscriptArtifacts(props.sessionId, verifiedOpenTargets);
  ...
  const buildDraft = useCallback(
    (text: string, nextAttachments: ComposerAttachment[]): ComposerDraft => {
      ...
      resolved = resolved.replace(
        /\[skill ([^\]]+)\]/g,
        (_match, name: string) => `the \"${name}\" skill`,
      );
  ...
  const queuedMessages = useMemo(
    () =>
      queuedDrafts.map((draftItem) => {
        ...
        if (text) return text;
        return t("composer.queued_attachments_only", {
          count: draftItem.attachments.length,
        });
      }),
    [queuedDrafts],
  );
  ...
  const handleInsertMention = (kind: ComposerMentionKind, value: string) => {
    ...
    if (kind === "app") {
      void (async () => {
        try {
          const status =
            (await desktopBridge.checkComputerUsePermissions()) as {
              ok?: boolean;
            };
          if (status.ok === true) return;
          toast.warning(
            t("composer.computer_use_permissions_missing", { app: value }),
            {
              action: {
                label: t("composer.computer_use_permissions_setup"),
                onClick: () =>
                  void desktopBridge.openComputerUsePermissionSetup(),
              },
            },
          );
        } catch {
    ...
    () => ({
      ...
      args: [
        {
          ...
          type: "string",
          required: true,
          description: "Prompt text to place in the composer.",
        },
      ],
      ...
      execute: async (args, helpers) => {
        ...
        helpers.setNarration(
          `Typing ${text.length.toLocaleString()} characters into the composer…`,
        );
    ...
    if (directory && servers.length) {
      void attemptSilentMcpReauth({
        ...
        .then(async (attempted) => {
          if (!attempted) return;
          const healed = unwrap(
            await opencodeClient.mcp.status({ directory }),
          ) as McpStatusMap;
  ...
  const listImportedPlugins = async (): Promise<CloudImportedPlugin[]> => {
    ...
    const plugins = Object.values(
      readWorkspaceCloudImports(response.openwork).plugins,
  ...
  useEffect(() => {
    ...
    if (
      ...
      !isSessionSurfaceMounted(state.sessionId)
    ) {
      state.closeFind();
    }
  }, [props.sessionId]);
  ...
  const handleMessageListDispatchAction = useCallback(
  ...
  const handleMessageListSetPrompt = useCallback(
    (prompt: string) => {
  ...
  const handleRevertToUserMessage = useCallback(
    (messageId: string) => {
  ...
  const handleForkAtMessage = useCallback(
    (messageId: string) => {
      ...
      props.onForkAtMessage?.(
        resolveForkBoundaryId(renderedMessages, messageId),
        props.sessionId,
      );
    },
  ...
  const handleEditUserMessage = useCallback(
    (messageId: string, text: string) => {
      void (async () => {
        ...
        const reverted = await props.onRevertToMessage?.(
          messageId,
          props.sessionId,
        );
  ...
  const sessionScrollTopControlAction = useMemo<OpenworkControlAction>(
    () => ({
      ...
      execute: () => {
        ...
        if (!container)
          return { ok: false, error: "Session transcript is not mounted" };
    ...
    () => ({
      ...
      execute: () => {
        ...
        if (!message)
          return {
            ok: false,
            error: "No messages are visible in this session",
          };
        return {
          ok: true,
          sessionId: props.sessionId,
  ...
  const sessionReadTranscriptControlAction = useMemo<OpenworkControlAction>(
    () => ({
      ...
      execute: (args) => {
        ...
        if (!slice.length)
          return { ok: false, error: "No messages in this session" };
        return {
          ok: true,
          sessionId: props.sessionId,
          messageCount: total,
                    ...
                    <EnvironmentVariableProvider
                      client={
                        props.isRemoteWorkspace
                          ? null
                          : (props.environmentClient ?? props.client)
                      }
                    ...
                    >
                      <MessageListProvider
                        ...
                        providerConnectedCount={
                          props.providerConnectedCount ?? 0
                        }
                        ...
                        onSaveTestPlan={props.onSaveTestPlan}
                      >
                        <MessageList
                          messages={renderedMessages}
                          status={status}
                          retryStatus={
                            liveStatus.type === "retry" ? liveStatus : null
                          }
                        />
                      </MessageListProvider>
                    </EnvironmentVariableProvider>
        ...
        <div ref={composerShellRef} className="shrink-0 px-0 pb-2 pt-2">
          {(props.providerConnectedCount ?? 0) === 0 ? (
            ...
            >
              ...
              <span className="text-amber-11/70">
                Add a provider to run tasks.
              </span>
            </button>
          ) : null}
                      ...
                      <QuestionPanel
                        ...
                        onReply={(answers) => {
                          if (props.activeQuestion) {
                            props.respondQuestion?.(
                              props.activeQuestion.id,
                              answers,
                            );
                          }
                        }}
