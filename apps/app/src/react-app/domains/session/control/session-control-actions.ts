...
export function useSessionControlActions(input: UseSessionControlActionsInput) {
      ...
      args: [
        {
          ...
          type: "string",
          required: true,
          description: "Session ID from session.list_sessions.",
        },
      ],
      execute: (args) => {
  ...
  const deleteSessionControlAction = useMemo<OpenworkControlAction>(
    () => ({
      ...
      execute: async (args) => {
        ...
        if (!openworkClient)
          return { ok: false, error: "Sprintnex server is not connected" };
        ...
        if (!targetWorkspace)
          return {
            ok: false,
            error: "Session was not found in the current session list",
          };
  ...
  const pinControlAction = useMemo<OpenworkControlAction>(
    () => ({
      ...
      description:
        "Toggle pin on a session. Pinned sessions float to the top of the sidebar.",
      sideEffect: "mutation",
      requiresArgs: true,
      args: [
        {
  ...
  const archiveControlAction = useMemo<OpenworkControlAction>(
    () => ({
      ...
      description:
        "Archive a session (non-destructive, preserves context). Archived sessions move to the Archived section. Pass archived=false to unarchive.",
      sideEffect: "mutation",
      requiresArgs: true,
      args: [
        {
  ...
  const groupCreateControlAction = useMemo<OpenworkControlAction>(
    () => ({
      ...
      description:
        "Create a new group (folder/separator) in the current workspace sidebar. Sessions can then be moved into it.",
      sideEffect: "mutation",
      requiresArgs: true,
      args: [
        {
          ...
          type: "string",
          required: true,
          description: "Group name (e.g. 'Done', 'In progress', 'Backlog').",
        },
        {
          name: "workspaceId",
          type: "string",
  ...
  const groupMoveControlAction = useMemo<OpenworkControlAction>(
    () => ({
      ...
      description:
        "Assign a session to a group (folder). Pass groupId=null or omit to remove from current group. Use session.group.list to see available groups.",
      sideEffect: "mutation",
      requiresArgs: true,
      args: [
        {
        ...
        },
        {
          ...
          type: "string",
          required: false,
          description: "Group ID to move into. Omit or null to ungrouped.",
        },
        {
          name: "workspaceId",
          type: "string",
          required: false,
          description: "Workspace ID. Defaults to session's workspace.",
        },
      ],
      execute: (args) => {
        const sessionId = stringArg(args, "sessionId");
        ...
        const wsId =
          resolveWorkspaceId(stringArg(args, "workspaceId")) ||
          targetWorkspace?.id;
  ...
  const groupRemoveControlAction = useMemo<OpenworkControlAction>(
    () => ({
      ...
      description:
        "Remove a group from the workspace. Sessions in the group become ungrouped (not deleted).",
      sideEffect: "mutation",
      requiresConfirmation: true,
      requiresArgs: true,
      args: [
        ...
        },
        {
          ...
          type: "string",
          required: false,
          description: "Workspace ID. Defaults to selected.",
        },
        {
          name: "confirmed",
          type: "boolean",
