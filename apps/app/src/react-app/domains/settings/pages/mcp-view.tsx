...
const serviceIcon = (name: string) => {
  ...
  if (lower.includes("openwork") && lower.includes("ui"))
    return MonitorSmartphone;
  return Plug2;
};
...
export function McpView(props: McpViewProps) {
  ...
  const [detailOrgMcpItem, setDetailOrgMcpItem] =
    useState<ExtensionItem | null>(null);
  const [openworkUiMcpCommand, setOpenworkUiMcpCommand] = useState<
    string[] | null
  >(null);
  const [openworkUiMcpEnvironment, setOpenworkUiMcpEnvironment] =
  ...
  const launchCommandForEntry = (entry: McpDirectoryInfo) => {
    if (entry.serverName === "openwork-ui")
      return openworkUiMcpCommand ?? undefined;
  ...
  const hiddenCount =
    quickConnectList.filter((entry) => isOpenWorkExtensionHidden(entry))
      .length +
    (props.installedSkills ?? []).filter((skill) =>
      isOpenWorkExtensionHidden(getSkillHiddenId(skill)),
    ).length +
    (props.installedPlugins ?? []).filter((plugin) =>
      isOpenWorkExtensionHidden(`plugin:${plugin.pluginId}`),
    ).length;
  const policyHiddenBuiltInCount = props.builtInExtensionsDisabled
    ? quickConnectList.filter(
        (entry) =>
          isBuiltInOpenWorkExtension(entry) &&
    ...
    <section className="space-y-8 max-w-3xl w-full animate-in fade-in duration-300">
      ...
      {props.builtInExtensionsDisabled ? (
        <div className="rounded-xl border border-amber-6 bg-amber-2 px-4 py-3 text-xs text-amber-11">
          Built-in OpenWork extensions are disabled by your organization. Use
          Show hidden to review blocked built-ins.
        </div>
      ) : null}
      ...
      <McpQuickConnectSection
        entries={quickConnectList.filter((entry) => {
          if (
            !showHidden &&
            (isOpenWorkExtensionHidden(entry) ||
              (props.builtInExtensionsDisabled &&
        ...
        installedSkills={(props.installedSkills ?? []).filter((skill) => {
          if (!showHidden && isOpenWorkExtensionHidden(getSkillHiddenId(skill)))
            return false;
        ...
        installedOrgMcpItems={(props.installedOrgMcpItems ?? []).filter(
          (item) => {
        ...
        isConfigured={(entry) => {
          ...
          if (isToggleOnlyExtension(entry))
            return isOpenWorkExtensionEnabled(entry);
      ...
      {detailEntry
        ? (() => {
            ...
            const disabledReason =
              props.builtInExtensionsDisabled &&
              isBuiltInOpenWorkExtension(detailEntry)
                ? builtInExtensionDisabledReason
                : null;
            ...
            return (
              <ExtensionDetailModal
                ...
                environment={
                  detailEntry.serverName === "openwork-ui"
                    ? (openworkUiMcpEnvironment ?? undefined)
                    : undefined
                }
                url={
      ...
      {detailSkill
        ? (() => {
            const hidden = isOpenWorkExtensionHidden(
      ...
      {detailOrgMcpItem && isOrgMcpConnectionItem(detailOrgMcpItem)
        ? (() => {
            ...
            const canDisconnect =
              canDisconnectNativeProviderAccount(connection);
            return (
              <ExtensionDetailModal
                open={true}
                ...
                onUninstall={
                  canDisconnect && props.disconnectOrgMcp
                    ? () => props.disconnectOrgMcp?.(connection.id)
                    : undefined
                }
                ...
                configSlot={
                  <div className="flex flex-wrap gap-2">
                    <span className="rounded-full border border-dls-border bg-dls-hover px-2 py-1 text-xs text-dls-secondary">
    ...
    <div className="rounded-2xl border border-blue-6/30 bg-[linear-gradient(180deg,rgba(59,130,246,0.08),rgba(59,130,246,0.03))] p-5 sm:px-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="space-y-1">
          <div className="text-base font-semibold text-dls-text">
        ...
        {props.entries.map((entry) => {
          ...
          return (
            <ExtensionCard
              ...
              disabled={props.busy}
              actionLabel={
                configured ? "View details" : t("mcp.tap_to_connect")
              }
            ...
            return (
              <div key={item.id} className="space-y-2">
                <ExtensionCard
                  name={item.name}
                  description={
                    item.description ?? "Shared by your organization."
                  }
                  kind="mcp"
                  url={connection.url}
                  connected={true}
        ...
        (props.installedOrgMcpItems ?? []).length === 0 ? (
          <div className="col-span-full rounded-xl border border-dashed border-dls-border px-5 py-10 text-center">
            ...
            <div className="mt-1 text-xs text-dls-secondary/60">
              Try a different search, filter, or open Marketplace to add one.
            </div>
          </div>
        ) : null}
      </div>
...
function McpConfiguredServerRow(props: {
  ...
  return (
    <div
      className={`rounded-xl border transition-all ${props.selected ? "border-blue-7 bg-blue-2 shadow-sm" : "border-dls-border bg-dls-surface hover:bg-dls-hover"}`}
    >
      <button
        type="button"
      ...
      >
        <div className="flex items-center gap-3">
          <div
            className={`flex size-8 shrink-0 items-center justify-center rounded-lg border ${props.status === "connected" ? "border-green-6 bg-green-3" : serviceIconBg(props.entry.name)}`}
          >
            <Icon
              size={15}
              className={
