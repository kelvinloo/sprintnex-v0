...
type MarketplaceRow =
  ...
  | BuiltInMarketplaceRow
  | OrgMcpMarketplaceRow;

export function shouldShowMarketplaceRows(
  isSignedIn: boolean,
  activeOrgId: string,
) {
...
function pluginComposition(plugin: DenOrgPlugin) {
  const componentEntries = Object.entries(plugin.componentCounts).flatMap(
    ([type, count]) => {
      if (count <= 0) return [];
...
function pluginManifestSearchText(plugin: DenOrgPlugin) {
  ...
  return [
    ...
    ...manifest.resources.map(
      (resource) =>
        `${resource.id} ${resource.label ?? ""} ${resource.description ?? ""}`,
    ),
    ...(manifest.contributions?.map(
      (contribution) => `${contribution.ref ?? ""} ${contribution.label ?? ""}`,
    ) ?? []),
  ].join(" ");
}
...
export function CloudMarketplacesView({
  ...
  const [updateAllProgress, setUpdateAllProgress] = React.useState<{
    current: number;
    total: number;
  } | null>(null);
  ...
  const [highlightPluginName, setHighlightPluginName] = React.useState<
    string | null
  >(null);
  ...
  React.useEffect(() => {
    ...
    return () =>
      window.removeEventListener(openMarketplacePluginEvent, handler);
  }, []);
  ...
  const extensionItemsByBuiltInId = React.useMemo(
    () =>
      new Map(
        extensionItems.flatMap((item) =>
          item.builtInEntry
            ? [
                [
                  item.builtInEntry.id ??
                    item.builtInEntry.serverName ??
                    item.builtInEntry.name,
                  item,
                ] as const,
              ]
            : [],
  ...
  const extensionItemsByPluginId = React.useMemo(
    () =>
      new Map(
        extensionItems.flatMap((item) =>
          item.plugin ? [[item.plugin.id, item] as const] : [],
        ),
      ),
    [extensionItems],
  );
  ...
  const cloudRows = React.useMemo<MarketplacePackageRow[]>(() => {
    return marketplaces.flatMap((marketplace) =>
      marketplace.plugins.flatMap((plugin) => {
        if (
          connectEnabled === true &&
          !isDesktopInstallableMarketplacePlugin(plugin)
        )
          return [];
        ...
        const status: MarketplacePackageStatus =
          imported &&
          pendingChanges[plugin.id] === "modified" &&
          !isCloudBuiltInPlugin(plugin)
            ? "update_available"
            : (item?.installState ??
              (isCloudBuiltInPlugin(plugin)
                ? "installed"
                : pluginStatus(imported, plugin)));
        return [
          {
            source: "cloud",
  ...
  const builtInRows = React.useMemo<BuiltInMarketplaceRow[]>(() => {
    return builtInEntries.map((entry) => {
      const item = extensionItemsByBuiltInId.get(
        entry.id ?? entry.serverName ?? entry.name,
      );
      const enablement =
        entry.extensionManifest?.enablement && enablementContext
          ? evaluateEnablement(
              entry.extensionManifest.enablement,
              enablementContext,
            )
          : null;
      const active =
        item?.active ??
        enablement?.active ??
        isBuiltInConnected?.(entry) ??
        false;
      return {
        source: "built-in",
        marketplaceId: "openwork-builtins",
        marketplaceName: "Sprintnex Built-ins",
        entry,
        active,
        ...
        searchableText: [
          ...
          ...(entry.extensionManifest?.resources.map(
            (resource) => `${resource.id} ${resource.label ?? ""}`,
          ) ?? []),
        ]
          .join(" ")
          .toLowerCase(),
  ...
  const orgMcpRows = React.useMemo<OrgMcpMarketplaceRow[]>(() => {
    ...
    return extensionItems.flatMap((item) => {
      if (!isOrgMcpConnectionItem(item) || item.installState !== "available")
        return [];
  ...
  const rows = React.useMemo<MarketplaceRow[]>(
    () => (canShowRows ? [...builtInRows, ...cloudRows, ...orgMcpRows] : []),
    [builtInRows, canShowRows, cloudRows, orgMcpRows],
  );

  React.useEffect(() => {
    if (detailRow?.source !== "org-mcp") return;
    const current = orgMcpRows.find(
      (row) => row.connection.id === detailRow.connection.id,
    );
    if (!current) {
      setDetailRow(null);
      return;
      ...
      canShowRows
        ? [
            ...(builtInRows.length > 0
              ? [{ id: "sprintnex-builtins", name: "Sprintnex Built-ins" }]
              : []),
            ...
            ...(orgMcpRows.length > 0
              ? [
                  {
                    id: "org-mcp-connections",
                    name: "Organization MCP Connections",
                  },
                ]
              : []),
          ]
        ...
        if (!quiet) {
          setActionError(
            error instanceof Error
              ? error.message
              : "Failed to load marketplace extensions.",
          );
        }
      } finally {
        setBusy(false);
    ...
    () =>
      cloudRows.filter(
        (row) =>
          ...
          !isCloudBuiltInPlugin(row.plugin),
      ),
    [cloudRows],
  );
  ...
  const removedUpstreamPlugins = React.useMemo(
    () =>
      Object.values(importedPlugins).filter(
        (plugin) => pendingChanges[plugin.pluginId] === "removed",
      ),
  ...
  const updateAll = React.useCallback(async () => {
    ...
    if (failed > 0) {
      setActionError(
        `Failed to update ${failed} extension${failed === 1 ? "" : "s"}.`,
      );
    } else if (warnings.length > 0) {
      toast.warning(
        `Updated ${targets.length} extension${targets.length === 1 ? "" : "s"}. ${warnings.join(" ")}`,
      );
    } else if (targets.length > 0) {
      toast.success(
        `Updated ${targets.length} extension${targets.length === 1 ? "" : "s"}.`,
      );
    }
    ...
    <SettingsSection>
      <SettingsSectionHeader>
        <SettingsSectionHeaderContent>
          <SettingsSectionHeaderTitle>
            {connectEnabled === true
              ? t("extensions.marketplace_local_title")
              : t("extensions.marketplace_title")}
          </SettingsSectionHeaderTitle>
          <SettingsSectionHeaderDescription>
            {connectEnabled === true
              ? t("extensions.marketplace_local_description")
        ...
        <SettingsSectionHeaderActions>
          {updatableRows.length >= 2 ? (
            ...
            >
              {updateAllProgress
                ? t("extensions.update_all_progress", {
      ...
      {!isSignedIn ? (
        <SettingsNotice>
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <span>
              You can use OpenWork without an account. Sign in to OpenWork Cloud
              to load the Marketplace, including OpenWork's built-in extensions
              and any organization marketplaces.
            </span>
      ...
      {removedUpstreamPlugins.map((plugin) => (
        <SettingsNotice key={plugin.pluginId}>
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            >
              {actionId === plugin.pluginId
                ? "Working..."
                : t("extensions.remove_from_workspace_button")}
            </Button>
          </div>
        </SettingsNotice>
      ))}
            ...
            <div className="absolute right-0 z-20 mt-2 w-72 rounded-xl border border-dls-border bg-dls-surface p-3 shadow-[var(--dls-shell-shadow)]">
              <label className="grid gap-1.5 text-xs text-dls-secondary">
                ...
                <select
                  ...
                  onChange={(event) =>
                    setMarketplaceFilter(event.currentTarget.value)
                  }
                >
      ...
      {!busy && displayRows.length === 0 ? (
        <SettingsListEmptyState>
          {!isSignedIn
            ? "Sign in to view marketplace extensions."
            : activeOrgId
            ...
            const pluginName =
              row.source === "cloud"
                ...
                : row.source === "built-in"
                  ...
                  : row.item.name;
            const isHighlighted =
              highlightPluginName != null && pluginName === highlightPluginName;
            return (
              <MarketplaceCard
                key={
                  row.source === "cloud"
                    ? `${row.marketplaceId}:${row.plugin.id}`
                    : row.source === "built-in"
                      ? `${row.marketplaceId}:${row.entry.id ?? row.entry.name}`
                      : row.item.id
                }
                actionId={actionId}
                row={row}
...
function MarketplaceCard(props: {
  ...
  if (row.source === "built-in") {
    ...
    const entryUrl =
      typeof row.entry.url === "string" ? row.entry.url : undefined;
    return (
      <div ref={highlightRef} className={highlightClass}>
        <ExtensionCard
          ...
          disabledReason={
            props.builtInDisabled ? "Disabled by organization" : null
          }
    ...
    return (
      <div ref={highlightRef} className={`space-y-2 ${highlightClass}`}>
        <ExtensionCard
          name={row.item.name}
          description={
            row.item.description ?? "Available from your organization."
          }
          kind="mcp"
          ...
          actionLabel={
            actionBusy
              ...
              : disconnecting
                ...
                : ready
                  ? "View details"
                  : orgMcpConnectionActionLabel(row.connection)
          }
          onClick={() => onOpenDetail(row)}
        />
        {canDisconnect ? (
          ...
          >
            {disconnecting
              ? t("mcp.org_connection_disconnecting_action")
              : t("mcp.org_connection_disconnect_action")}
          </Button>
        ) : null}
      </div>
    );
  ...
  const deliveryAction = resolveMarketplaceDeliveryAction({
    connectEnabled:
      props.connectEnabled === true &&
      !isDesktopInstallableMarketplacePlugin(row.plugin),
    importedLocally: Boolean(row.imported),
  });
  const cloudDelivery = deliveryAction !== "install";
  const needsSetup = Boolean(
    row.imported && row.item?.setupState === "needs_setup",
  );
  ...
  return (
    <div ref={highlightRef} className={`flex flex-col gap-2 ${highlightClass}`}>
      <ExtensionCard
        ...
        connected={
          cloudBuiltIn ||
          (cloudDelivery && !needsSetup) ||
          (Boolean(row.imported) && !needsSetup)
        }
        connectedLabel={
          cloudDelivery && !needsSetup
...
function BuiltInMarketplaceDetailModal(props: {
  ...
  const { row, disabled, connecting, configSlot, onClose, onSetEnabled } =
    props;
  ...
  return (
    <ExtensionDetailModal
      ...
      onConnect={
        !disabled && toggleControlled && !row.active && onSetEnabled
          ? () => onSetEnabled(entry, true)
          : undefined
      }
      onUninstall={
        !disabled && toggleControlled && row.active && onSetEnabled
          ? () => onSetEnabled(entry, false)
          : undefined
      }
    />
  );
...
function OrgMcpConnectionDetailModal(props: {
  ...
  return (
    <ExtensionDetailModal
      ...
      showEnablementCard={false}
      onConnect={
        !ready && onConnect ? () => onConnect(row.connection.id) : undefined
      }
      onUninstall={
      ...
      configSlot={
        <div className="space-y-4">
          <div className="flex flex-wrap gap-2">
            ...
            <SettingsPill>
              {row.connection.credentialMode === "shared"
                ? "Org account"
                : "Your account"}
            </SettingsPill>
          ...
          <SettingsNotice>
            OpenWork stores this sign-in in the organization cloud. Once
            connected, your desktop agent can use the tools through OpenWork
            Cloud Control.
          </SettingsNotice>
        </div>
      }
...
function MarketplacePackageDetailModal(props: {
  ...
  const importedExternalConnectionIds =
    row.imported?.files.flatMap((file) =>
      file.externalMcpConnectionId ? [file.externalMcpConnectionId] : [],
    ) ?? [];
  const importedConnections = [
    ...new Set(importedExternalConnectionIds),
  ].flatMap((connectionId) => {
    const connection = orgMcpConnections.find(
      (entry) => entry.id === connectionId,
    );
    return connection ? [connection] : [];
  });
  const missingImportedConnectionCount =
    new Set(importedExternalConnectionIds).size - importedConnections.length;
  ...
  return (
    <ExtensionDetailModal
      ...
      resourceLabels={
        manifest?.resources.map((resource) => resource.label ?? resource.id) ??
        []
      }
      contributionLabels={
        manifest?.contributions?.map(
        ...
        <div className="space-y-4">
          <div className="flex flex-wrap gap-2">
            <SettingsPill
              className={
                needsSetup
                  ? "border-amber-7/30 bg-amber-3/20 text-amber-11"
            ...
            <div className="mt-2 grid gap-2">
              {row.composition.map((entry) => (
                ...
                >
                  ...
                  <span className="rounded-full bg-dls-surface px-2 py-0.5 text-xs font-medium text-muted-foreground">
                    {entry.count}
                  </span>
                </div>
              ))}
          ...
          {missingImportedConnectionCount > 0 ? (
            <SettingsNotice tone="error">
              ...
              {missingImportedConnectionCount === 1
                ? "one required MCP connection"
                : `${missingImportedConnectionCount} required MCP connections`}
              . Ask an admin to update the connection sharing settings.
            </SettingsNotice>
          ) : null}
          {importedConnections.length > 0 ? (
            <div className="rounded-xl border border-dls-border bg-dls-hover px-3 py-3">
              <div className="text-xs font-semibold uppercase tracking-[0.14em] text-muted-foreground">
                Cloud MCP connections
              </div>
              <div className="mt-3 grid gap-2">
                {importedConnections.map((connection) => {
                  const ready = isOrgMcpConnectionReady(connection);
                  const needsMemberConnect =
                  ...
                  return (
                    ...
                    >
                      <summary className="cursor-pointer text-sm font-medium text-card-foreground">
                        <span className="uppercase text-[10px] tracking-[0.12em] text-muted-foreground">
                          {object.objectType}
                        </span>{" "}
                        {object.title}
                      </summary>
