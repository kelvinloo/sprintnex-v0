...
export function ArchitectureMismatchGate({ children }: ArchitectureMismatchGateProps) {
  ...
  useEffect(() => {
    let cancelled = false;
    const bridge = window.__SPRINTNEX_ELECTRON__?.system?.getArchitectureInfo;
    if (!bridge) {
  ...
  const openDownload = useCallback(() => {
    ...
    if (!url) return;
    void window.__SPRINTNEX_ELECTRON__?.shell?.openExternal?.(url);
  }, [info?.downloadUrl, info?.releaseUrl]);

  const openRelease = useCallback(() => {
    if (!info?.releaseUrl) return;
    void window.__SPRINTNEX_ELECTRON__?.shell?.openExternal?.(info.releaseUrl);
  }, [info?.releaseUrl]);
