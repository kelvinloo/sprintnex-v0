 ...
 * missing input.todos.
 */
class ToolMessage extends React.Component<
  ToolMessageProps,
  { failed: boolean }
> {
  state = { failed: false };
...
function FileMessage({ part }: FileMessageProps) {
  ...
  if (isImage) {
    return <Image src={part.url} alt={title} loading="lazy" decoding="async" />;
  }
...
function EmptyMessage({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      className={cn(
...
const USER_SKILL_TOKEN_RE =
  /(Load \[skill [^\]]+\] and follow its instructions\.|\[skill [^\]]+\])/;
...
function renderPlainTextWithSearchHighlights(
  text: string,
...
function renderUserTextWithSkillChips(
  text: string,
  highlightQuery: string | undefined,
) {
  if (!USER_SKILL_TOKEN_RE.test(text))
    return renderPlainTextWithSearchHighlights(text, highlightQuery, "text");
  let offset = 0;
  return text.split(USER_SKILL_TOKEN_RE).map((segment) => {
    const key = `${offset}:${segment}`;
    offset += segment.length;
    const skillMatch = segment.match(
      /^(?:Load )?\[skill ([^\]]+)\](?: and follow its instructions\.)?$/,
    );
    if (skillMatch?.[1])
      return <UserSkillChip key={key} name={skillMatch[1]} />;
    return (
      <React.Fragment key={key}>
        {renderPlainTextWithSearchHighlights(segment, highlightQuery, key)}
      </React.Fragment>
    );
  });
}

const UserMessage = React.memo(({ message, isStreaming }: UserMessageProps) => {
  const {
                  ...
                  {messageText ? (
                    <MessageAction tooltip="Edit message">
                      <Button
                        ...
                        onClick={() =>
                          onEditUserMessage(message.id, messageText)
                        }
                      >
                        <Pencil />
                      </Button>
...
const MessageComponent = React.memo(
  ({
    ...
    if (isSessionErrorMessage(message)) {
      return (
        <ErrorMessage error={getMessagesText([message]) || "Session failed"} />
      );
    }
...
function MessageGroup({
  ...
  const lastRealItem = items.findLast(
    (item) => !isSessionErrorMessage(item.message),
  );
  const isLiveGroup =
    isStreaming &&
    lastItem !== undefined &&
  ...
  while (
    ...
    !getRenderableMessage(items[stepCount].message)
  ) {
    stepCount += 1;
  }
  ...
  return (
    <div className="flex flex-col gap-2 group/message-group">
      ...
      {proseItems.map((item, groupIndex) =>
        renderItem(item, stepItems.length + groupIndex),
      )}
              ...
              <>
                ...
                <MessageAction tooltip="Revert">
                  <Button
                    ...
                    aria-label="Revert"
                    onClick={() =>
                      onRevertToUserMessage(lastRealItem.message.id)
                    }
                  >
                    <Undo2 />
                  </Button>
...
export function MessageList({
  ...
  const items = React.useMemo(
    () => groupMessages(messages, status),
    [messages, status],
  );
  const error = useSessionErrorMessage();
  const hasSessionErrorMessage = React.useMemo(
    () => messages.some(isSessionErrorMessage),
    [messages],
  );
    ...
    <div className={cn("flex flex-col gap-2 @container/message-list")}>
      ...
      {items.map((item) => {
        ...
        const isLastStep =
          ...
          messages[item.index + 1].role !== item.message.role;
