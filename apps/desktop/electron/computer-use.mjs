...
function computerUseHelperExecutablePath() {
  ...
  const explicitBinary = process.env.SPRINTNEX_COMPUTER_USE_BINARY?.trim();
  const candidates = [
    explicitBinary,
...
function computerUseHelperAppPath() {
  const explicitApp = process.env.SPRINTNEX_COMPUTER_USE_APP?.trim();
  const candidates = [
    explicitApp,
...
function resolveComputerUseExecutable() {
  ...
  const explicit = process.env.SPRINTNEX_COMPUTER_USE_BINARY?.trim();
