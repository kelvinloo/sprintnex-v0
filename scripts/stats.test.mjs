...
test("classifyAsset buckets updater and non-desktop assets separately", () => {
  ...
  const sidecarRelease = { tag_name: "sprintnex-orchestrator-v0.11.135" }
  ...
  assert.equal(classifyAsset(sidecarRelease, { name: "sprintnex-server-darwin-arm64" }), "other")
})
...
test("calculate aggregates legacy total and v2 buckets", () => {
  const releases = [
    ...
    {
      tag_name: "sprintnex-orchestrator-v0.11.135",
      assets: [{ name: "sprintnex-server-darwin-arm64", download_count: 8 }],
    },
  ]
