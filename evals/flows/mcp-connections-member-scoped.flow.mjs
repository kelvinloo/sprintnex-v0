 ...
 * - den-api at SPRINTNEX_EVAL_DEN_API_URL, den-web at SPRINTNEX_EVAL_DEN_WEB_URL.
 * - Mock OAuth+MCP server reachable at MOCK_OAUTH_MCP_URL from den-api.
 * - Admin: SPRINTNEX_EVAL_DEMO_EMAIL/PASSWORD (default seeded alex@acme.test).
 * - Member: SPRINTNEX_EVAL_MEMBER_EMAIL/PASSWORD (default
 ...
 *   SPRINTNEX_EVAL_MARK_VERIFIED_CMD — a shell template with {email}, e.g.
      ...
      run: async (ctx) => {
        ...
        if (!state.memberSession) {
          ...
          ctx.assert(
            ...
            "Member email must be verified to accept the invitation; set SPRINTNEX_EVAL_MARK_VERIFIED_CMD (shell template with {email}).",
          );
